...
- **Summary / weekly / range reports**: revenue (= follow-up payments collected), expenses (paid + postponed — lab cost is auto-recorded here, so it is included in this figure rather than shown as a separate report line), profit (= revenue − expenses), clinic gross profit (= Σ price − discount − lab), patient count for any date range — all scoped to non-deleted follow-ups. Every report also shows a **current "Amounts Still Owed" table** — what each patient still owes (net billed = price − discount, paid, left, last visit, overdue days) — plus an *Unpaid by Patients* total
