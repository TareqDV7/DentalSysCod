...
Staff manage it from **Settings → Cloud Sync** with a single **on/off toggle** — there's no cloud URL or serial to type. Flipping it on calls `POST /api/cloud/enable`, which reads the `active_serial_number` + `active_serial_token` that were saved when the server was activated, resolves the cloud node from the baked `_BAKED_CLOUD_BASE_URL` (`https://app.dentacare.tech`, overridable via `CLINIC_LICENSE_CLOUD_URL` / `CLINIC_CLOUD_URL` for staging or self-host), registers the clinic on the cloud, stores the returned token, and runs an immediate first sync (returns `409` if the server hasn't been activated yet). Flipping it off unpairs. The dashboard shows a small "☁️ Synced …" / "Cloud sync: off" / "⚠️ Cloud sync error" badge. The signed `offline_token` saved at activation is forwarded automatically, so registration succeeds against a cloud running with `CLINIC_REQUIRE_SIGNED_SERIAL=1`. (`POST /api/cloud/pair` — explicit cloud URL + serial + optional `offline_token` — remains as the underlying primitive for staging/self-host and is what `enable` calls through `_link_clinic_to_cloud`.)

**One-tap phone pairing (QR).** Once a clinic's cloud sync is on, **Settings → Cloud Sync → Link a phone** reveals a QR (`GET /api/cloud/pairing-qr`, login-gated, SVG) encoding the compact payload `{"v":1,"u":<cloud_url>,"t":<clinic_token>}`. The mobile app's **Settings → Scan QR to link** scans it (`mobile_scanner` → pure `parsePairingPayload()` → `AppState.linkWithToken()`), persists `cloud_url` + `clinic_token`, and runs a first sync — **without** re-typing the URL/serial and **without** re-registering, since the desktop already registered the clinic. Manual pairing remains as a fallback. The token is rendered only into the login-gated QR (served `Cache-Control: no-store`) and is never returned by the public `/api/cloud/status`. API: `POST /api/cloud/pair` · `GET /api/cloud/status` · `GET /api/cloud/pairing-qr` · `POST /api/cloud/sync-now` · `POST /api/cloud/unpair` (local server only; the cloud node returns `400`/`404`).
