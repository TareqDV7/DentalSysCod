...
├── tests/                    # 365 tests across 40 suites
...
1. **Vendor serial token (Ed25519).** Generated by `serial_generator.py` and signed with a vendor **private seed** that never leaves the vendor machine. The cloud node verifies it with the matching public key (`CLINIC_SERIAL_PUBLIC_KEY`) — this is what `/api/license/validate` and the cloud registration gate check. Generate the keypair once, then sign serials with it:

   ```bash
   python3 serial_generator.py --genkey            # → backend_ed25519_key.json (private seed; git-ignored)
   #   prints the public key → set CLINIC_SERIAL_PUBLIC_KEY=<that> on the cloud node
   ...
   Signing is mandatory — there is **no** demo/default key. See [`docs/SERIAL_GENERATOR_README.md`](docs/SERIAL_GENERATOR_README.md).
