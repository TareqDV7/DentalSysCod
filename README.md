...
- **Data tools (import / merge / replace / clear)** — **Settings → Data Tools** (local server, login required) adds five actions: **Export bundle** writes a `.zip` of a consistent DB snapshot plus the `uploads/` folder (so X-rays travel with it; in the desktop app it saves to an `exports/` folder beside the database and reveals it in the file manager, because the embedded WebView can't surface a browser download); **Merge another clinic** *additively* imports another DentaCare database — every incoming record is re-numbered and all its foreign keys (including the soft links `expenses.reference_id`→follow-up and `credit.invoice_id`→billing) are rewritten, catalogs are deduped by name, each dependent table is isolated so one problematic table is skipped with a warning instead of aborting the whole merge, and your existing data is **never overwritten**; **Replace database** swaps in an uploaded DB/bundle via a brief live maintenance window; **Clear catalogs** soft-deletes *every* procedure and tooth-condition (each deletion tombstoned, so the wipe propagates to the cloud node and paired phones on the next sync) while **patient data is kept untouched** — and catalogs also **ship empty on a fresh install** (no demo seed), so the doctor builds their own; **Find duplicate patients** scans for patients that share a normalized name (the dedupe a cross-clinic merge intentionally *doesn't* do, since two real people can legitimately share a name) and lists each group with per-patient record counts so you can either **merge** the duplicates into one survivor — reassigning every appointment, visit, treatment, plan, follow-up, bill, tooth-chart row, image and credit entry (`updated_at` triggers carry the change to paired devices), then deleting the empty shell (tombstoned so the deletion syncs) — or **delete** an empty duplicate outright via the existing `DELETE /api/patients/<id>`; merge takes a safety backup first and rolls back atomically on any error, and both actions report through a non-blocking toast (backed by `patient_dedupe.py`). A **safety backup is taken automatically before every merge or replace** and its path is returned. Merging X-rays requires the `.zip` bundle (a bare `.db` carries no image files); uploads are size-capped by `CLINIC_MAX_UPLOAD_MB` (default `1024`). Backed by `db_merge.py` (purely additive engine — current rows are never updated or deleted) and `db_import.py` (SQLite-magic validation + zip-slip-safe extraction); disabled on the cloud node.
...
The mobile sync banner reflects the **actual** transfer outcome. The reachability probe (`/api/system/readiness`) is an *open* endpoint (no clinic token), so a reachable cloud node with a missing/stale clinic token would previously still flash **"Synced"** while the authenticated `export`/`import` calls silently 401'd — "succeeded, but nothing moved." The banner now reports **"Sync failed"** unless **both** the pull and push legs complete (`InternetSyncService.syncAll` throws `SyncTransferException`; the leg rule lives in `lib/utils/sync_outcome.dart`), so a green "Synced" means data really transferred. (The desktop's own `cloud_last_sync_result` was already honest — it only records `ok` on real HTTP 200s both ways.)
...
- **One balance per patient (unified ledger)** — a patient has a single money position, `Outstanding = TotalCharged − TotalPaid`, summed across **both** the follow-up sheet **and** billing (each side can carry charges *and* payments). Positive = owed, zero = settled, negative = **credit** the clinic is holding for the patient. Credit is simply an overpayment (a negative outstanding) — there is no separate credit ledger and no "Use credit" step; an overpayment automatically offsets the next charge. It's computed by `get_patient_balance()` on the server (and `getPatientBalance` on mobile), the single source of truth that receivables, the patient list, the profile, payment history, and billing all derive from, so every surface shows the same number.
...
- **Odontogram (tooth chart)** — *temporarily hidden from the UI on both web and mobile pending the UI-overhaul redraw of the tooth silhouettes; the backend endpoints, `tooth_conditions`/`patient_tooth_chart`/`treatment_plan_teeth` tables, sync, and any saved data are all kept intact, and it is re-enabled by the `ODONTOGRAM_ENABLED` flag (web) / restoring the Tooth-chart tab (mobile).* — an interactive FDI permanent-dentition chart (32 teeth, ISO two-digit numbering 11–48, **clinician's view locked left-to-right in both English and Arabic RTL**) on the patient profile. Each tooth can carry **one or more conditions at once** (e.g. a cavity *and* a crown), **each with its own note**, drawn from an **editable catalog** (`tooth_conditions` — EN/AR, color-coded, managed like the procedure catalog; **ships empty**, the doctor adds their own). A tooth with several conditions renders as **stacked color bands**; tap a tooth to **multi-select the conditions that apply and describe each**, **+ Log treatment** (opens the follow-up Add form with the tooth pre-filled), or **+ Add to plan** (treatment plans are **multi-tooth**). Two badges are **computed at read time from the ledger + plans, never stored**: a purple *has-plan* dot and an amber *unpaid-balance* dot (₪). Teeth recorded on legacy follow-ups (`tooth_no`) are **auto-adopted** onto the chart on read (valid FDI only; junk ignored). Available on both the web portal (inline SVG arch) and the Flutter app (CustomPaint arch); the tables (`tooth_conditions`, `patient_tooth_chart` — now **multiple rows per tooth, one per condition** —, `treatment_plan_teeth`) ride the existing `SYNC_TABLES` machinery, so they sync (last-write-wins + tombstones) like everything else
...
- **Summary / weekly / range reports**: revenue (= payments collected across **both** the follow-up sheet and billing, so a payment recorded in either place counts once), expenses (paid + postponed — lab cost is auto-recorded here, so it is included in this figure rather than shown as a separate report line), profit (= revenue − expenses), clinic gross profit (= Σ price − discount − lab), patient count for any date range. Every report also shows a **current "Amounts Still Owed" table** — what each patient still owes (net charged across the sheet + billing, paid, left, last visit, overdue days) — plus an *Unpaid by Patients* total
- **Receivables report**: amount still owed per patient — charges and payments summed across **both** the follow-up sheet and billing, discounts subtracted, with patients who are settled or in credit excluded
...
- Billing / payment records with discount and balance due; payment method is a **Cash / Card / Transfer dropdown**. A billing entry can be a **charge, a payment, or both** — a payment-only receipt has subtotal 0 and just records cash taken in (an entry with neither a charge nor a payment is rejected). Paid status and balance are computed against the amount paid, clamped at 0. A billing charge raises, and a billing payment lowers, the patient's one unified balance — so billing and the follow-up sheet always reconcile to the same number. Deleting a billing record tombstones the deletion so it propagates on the next sync. Every patient picker (payments, statement, appointments) is a single **searchable dropdown** — one field that filters the list by name or phone as you type and slides the matches down (keyboard navigable), instead of a separate search box stacked above the select — so it stays usable with a large patient roster
...
> The dashboard's "Today's Revenue" and "Today's Visits" cards count *today's* payments and entries — revenue is summed across **both** the follow-up sheet and billing, the same basis as the mobile dashboard (visits are recorded on the follow-up sheet, not the legacy `visits` table). `Clinic profit = price − discount − lab expense`; lab expense is also auto-recorded as a postponed expense, so don't add the two together.

### Depo (Inventory / Stock)
- A desktop **Depo** / **مخزن** stock-management tab (Management group, between Catalog and Settings). Desktop is full management; the mobile app is view-only. Material cost is **insight-only** — it is shown as information (unit cost, on-hand value) and **never** affects clinic profit, expenses, or the follow-up billing math.
- **Items** — each stock item carries a name (EN/AR), category, a base unit and an optional pack unit + pack size (so anaesthetic can be counted in carpules with a box of 50, composite in compules, etc.), a low-stock threshold, optional reorder qty / supplier / location, and an opt-in **Track expiry** flag. The list shows on-hand quantity, **packs-remaining** (`quantity / pack_size`, display-only), a low-stock highlight, and total stock value. Items are **deactivated**, never hard-deleted. Unit cost is not typed in the editor — it is set by **Add stock** and blended with **weighted-average** costing, so the cost basis can't be bypassed.
- **Stock actions** — *Add stock* (quantity + unit cost, plus an expiry date when the item tracks expiry), *Adjust count* (recount to a counted quantity), and *Write-off* (breakage / contamination / expired). Every movement is posted through an append-only ledger that is the source of truth (`quantity = Σ change_qty`); a restock that lands at/below threshold raises a low-stock toast.
- **Procedure materials** — a sub-panel on the Procedure Catalog editor links stock items to a treatment procedure with a default consumption quantity, so recording that procedure knows what to deduct.
- **Follow-up "issued from stock"** — when a follow-up's procedure has linked materials, the form shows editable consumption rows pre-filled with the defaults; on save they deduct from stock and any low/zero stock surfaces as a **non-blocking** toast. A low or negative item never prevents saving a follow-up, and the billing/money fields are untouched. Deduction is **idempotent** on the follow-up id (editing or deleting the follow-up reverses and re-posts), so a follow-up can't double-deduct.
- **Stock report** — a low-stock list, total on-hand stock value, and an expiring-soon list (≤ 60 days, for expiry-tracked items).
- The three inventory tables (`inventory_items`, `procedure_materials`, `stock_movements`) ride the existing `SYNC_TABLES` machinery and are preserved across export-bundle, additive merge, and replace-database, like everything else.
...
- The active **license** (serial · clinic · plan · status · devices · expiry, with a copy-serial button) lives in **Settings → License** — it was moved off the top bar so the header stays uncluttered. The card reads the public `/api/license/status` (serial only, never the signed token) and re-renders on the language toggle
- Appointment status choices remain visible in both languages

### Access Control & Security
- The web portal (`/`), printable invoices (`/invoice/<id>`), the database backup download (`/api/backup`), and **every destructive Data Tools route** (`/api/data/export-bundle · merge · replace · clear-catalogs · duplicate-patients · merge-patients`) require a logged-in staff session — anonymous browsers are redirected to a sign-in page (API callers get `401`)
- Credentials live in a `users` table; passwords are stored as salted hashes (`werkzeug.security`). A default `admin` / `admin` account is seeded on first run (override with `CLINIC_ADMIN_PASSWORD`)
- **Forced first-run password change:** when the well-known `admin` / `admin` default is in use, the seeded account is flagged (`must_change_password`) and the portal redirects to a one-time `/change-password` screen before the SPA can load — so a fresh install can't be left running on the default credentials. Supplying `CLINIC_ADMIN_PASSWORD` up front seeds a real password and skips the prompt. The gate guards the local/LAN portal entry point (`/`); the cloud node relies on `CLINIC_ADMIN_PASSWORD` being set at deploy time, and the open data/sync API is never gated (mobile callers carry no staff session)
- Session secret is generated once and persisted in `app_settings`, so logins survive restarts
- **Baseline response headers** are set on every reply (`X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy: camera=(), microphone=(), geolocation=()`). `Strict-Transport-Security` is emitted **only over HTTPS** (the cloud node behind Caddy), never on plain-HTTP LAN access where a cached `max-age` would lock a clinic out of its own `http://` server. A strict Content-Security-Policy is deferred until the inline-everything `templates.py` is split into real templates + `static/` assets
- **CSRF protection:** every state-changing request (`POST`/`PUT`/`PATCH`/`DELETE`) must carry a per-session synchronizer token — sent as an `X-CSRFToken` header by the SPA (one `window.fetch` interceptor, token in a `<meta>` tag) or a hidden `csrf_token` field on the no-JS `/login` and `/change-password` forms (the token rotates on login). Requests authenticated by an `X-Clinic-Token` or `Authorization` **header** (the mobile app, cloud sync, vendor API) are exempt — a custom header can't be set by a cross-site form/simple request, so it proves the request isn't forged; the query-arg `clinic_token` does **not** exempt. Set `CLINIC_DISABLE_CSRF=1` to disable enforcement (default: enforced; logs a startup warning when off)
...
- **Roles & per-dentist accounts:** every user has a `role` — `admin`, `dentist`, or `staff` — plus a granular `user_permissions` grant table for anything narrower than the role default. A **dentist** account is confined server-side to their own `dentist_id` rows on appointments, follow-ups/billing, and reports (`dentist_scope()` — enforced in every scoped query, not a UI filter a client could bypass); legacy/front-desk rows with no `dentist_id` stay visible to everyone (shared), and patient records themselves are never scoped — only clinical/financial rows are. Mutating someone else's scoped row is a `403`. The **Manage Staff** screen (and its API) is role-aware: creating/editing accounts, assigning roles and permissions, and resetting a password all route through it
- **Sign-in:** a Windows-style **tile screen** lists active accounts — pick a tile, type the one shared password field, sign in. `LOCKOUT_THRESHOLD` (5) consecutive bad passwords locks that account for `LOCKOUT_MINUTES` (15) regardless of which tile is used
- **Email + verification:** any signed-in user can set/change their own email (Settings → Account); it starts unverified and a one-time 6-digit code is sent to confirm it (`/api/profile/email` + `/api/profile/email/verify`). Verified email is what enables **forgot-password** (`/api/login/forgot` requests a code, `/api/login/reset` redeems it) — the forgot endpoint always answers the same generic `{sent:true}`, win or lose, so it can't be used to enumerate accounts or emails
- **Staff invites:** an admin creating a staff account can supply an email with **no** password — that sends an invite email carrying a one-time code as the temporary password (forces a real password on first sign-in, and the email counts pre-verified since receiving/using the code already proves ownership)
- **Security alerts:** admins get an email whenever a security-relevant event happens on the account — a new staff account created, a password changed or reset, the recovery code redeemed — so an unattended admin inbox still sees a lockout escape or a new hire
- **Admin recovery code:** a printed one-time code (Settings → Security) that resets the oldest active admin's password with **no email or network involved** — the offline escape hatch when lockout, an unset mail relay, or no connectivity would otherwise strand the clinic. Redeeming it always issues a fresh code, so the clinic is never left without one
- **Offline-first by design:** every email send (`send_system_email`) is best-effort — an unpaired clinic, an unreachable relay, or a rate limit never blocks the underlying action (the account is still created, the password is still reset); the caller just gets `sent:false` and the outcome is logged server-side
...
- The Flutter app exposes a language toggle in Settings (EN / ع) via `app_state.dart`; the chosen **language and theme (light/dark)** are persisted to secure storage (`prefs_codec.dart` encodes them) and restored on the next cold start, so the app reopens in the mode you left it in instead of snapping back to English/light
...
├── auth_codes.py             # One-time code issue/verify (password reset, email verify, invites) + admin recovery code
├── email_templates.py        # Bilingual (EN/AR) plain-text system-email bodies, rendered by the cloud relay
...
├── tests/                    # 1132 tests across 120 suites
...
│   ├── test_credit_balance.py       # Unified patient balance (charged − paid across sheet + billing; credit = overpayment)
│   ├── test_date_utils.py
...
│   ├── test_patient_create_response.py # POST /api/patients echoes the created row (id + names) for mobile reconcile
...
│   ├── test_seed_empty.py            # Fresh DB ships empty catalogs (no demo seed; id=0 sentinel inactive)
│   ├── test_tooth_conditions.py      # Editable tooth-condition catalog (CRUD, soft-delete)
...
│   ├── test_data_tools_api.py      # /api/data/export-bundle · merge · replace · clear-catalogs (auth, cloud-disabled, size cap, safety backup, maintenance guard)
...
| Method | Endpoint | Description |
|--------|----------|-------------|
| GET / POST | `/login` | Sign-in page / submit credentials (form: `username`, `password`, `next`) — Windows-style tiles pick the account, one shared password field submits |
| GET | `/logout` | Clear session, redirect to sign-in |
| GET | `/api/auth/me` | Current session status (`{authenticated, username, permissions, role, email, email_verified}`) |
| POST | `/api/auth/change-password` | Change the signed-in user's password (`current_password`, `new_password`) |
| GET | `/api/dentists` | List active dentists (`id`, `display_name`) — used by dentist-filter pickers |
| GET / POST | `/api/staff` | List staff accounts / create one — requires login. Create takes `username`, `role` (`admin`/`dentist`/`staff`), optional `email` + `permissions`; supplying `email` with **no** `password` sends an **invite email** with a one-time code as the temporary password (`must_change_password` forces a real one on first sign-in, email counts pre-verified) |
| PUT | `/api/staff/<id>` | Update a staff account — role, active flag, display name, email (re-triggers verification), or a forced `new_password`. Refuses to deactivate the last account with `staff.manage` or demote the last active admin |
| GET / PUT | `/api/staff/<id>/permissions` | View / set a staff account's granular permission grants |
| POST | `/api/login/forgot` | Request a password-reset code by email (`identifier`) — always returns `{sent:true}` regardless of outcome, so the response can't be used to enumerate accounts |
| POST | `/api/login/reset` | Complete a password reset with the emailed code (`identifier`, `code`, `new_password`) |
| POST | `/api/login/recover` | Offline lockout escape: redeem the printed one-time admin recovery code (`code`, `new_password`) to reset the oldest active admin's password with no email involved; issues a fresh recovery code on redemption |
| POST | `/api/settings/recovery-code` | Generate/rotate the clinic's one-time admin recovery code — plaintext returned exactly once, only its hash persists (requires `staff.manage`) |
| POST | `/api/profile/email` | Self-service: the signed-in user sets/changes their own email, resets verification, and sends a code |
| POST | `/api/profile/email/verify` | Self-service: verify the pending email with the OTP code |
...
| DELETE | `/api/billing/<id>` | Delete record (tombstoned so the deletion syncs) |
...
| POST | `/api/data/export-bundle-file` | Write a `.zip` bundle to an `exports/` folder beside the DB and return its path (the desktop WebView can't surface a browser download) — login, local server only |
| POST | `/api/data/merge` | Additively merge an uploaded `.db`/`.zip` from another clinic; isolates each dependent table so one bad table can't abort the merge, takes a safety backup, returns a per-table report (with warnings) — login, local server only |
...
| POST | `/api/data/clear-catalogs` | Soft-delete + tombstone every procedure and tooth-condition; patient data untouched — login, local server only |
...
| POST | `/api/license/validate` | License authority — `{serial_token, device_fingerprint, device_name?}` → `{valid, reason?, status, plan_name, expires_at, grace_until, remaining_slots}`. Verifies the signature, registers on first use (caching the signed token), enforces status/subscription/renewal, and atomically claims a device slot. Business failures are HTTP 200 with `valid:false` (`reason` ∈ `malformed`/`bad_signature`/`revoked`/`suspended`/`expired`/`device_cap_reached`) |
| POST | `/api/license/claim` | **Short-serial online activation.** `{serial_number, device_fingerprint, device_name?}` → `{valid, serial_token, status, plan_name, expires_at, grace_until, max_devices, remaining_slots}`. Looks up the serial's **cached** signed token, applies the status/subscription gates, and atomically claims a device slot — so a clinic activates by typing just the short serial. `valid:false` with `reason` ∈ `not_found`/`no_token`/`revoked`/`suspended`/`expired`/`device_cap_reached`. Rate-limited (validate budget) |
| POST | `/api/license/admin/register-serial` | Admin-only (`X-Admin-Token`) — vendor pre-loads a signed serial so a brand-new clinic can claim by short serial alone. `{serial_token}` → `{success, serial, already_existed}`. Idempotent; verifies the Ed25519 signature |
...
| GET | `/api/license/admin/serials` | Admin-only (`X-Admin-Token`) **read-only registry** — lists every serial the cloud knows with live device usage: `{serials:[{serial, status, plan_name, clinic_name, max_devices, used_devices, has_token, issued_at, expires_at, grace_until, created_at, updated_at}], count}`. **Never** returns the signed token (`has_token` flag instead). Rate-limited; `401` when the token is unset or wrong |
...
| Method | Endpoint | Description |
|--------|----------|-------------|
| POST | `/api/license/activate` | Activate. `{serial_number}` (short, ~22 chars) → fetches the cached signed token from the cloud's `/api/license/claim`, verifies it locally, and caches it for offline use (internet needed once). `{serial_token}` → air-gapped paste path (verifies the full signed token locally, no network). `{serial_number, device_id}` → LAN-attach a device to a serial already activated on the clinic server. Returns the offline token |
...
1. **Vendor serial token (Ed25519).** Generated by `serial_generator.py` (CLI) or `serial_admin.py` (GUI), signed with a vendor **private seed** that never leaves the vendor machine. The local app and cloud verify it with the matching public key (`CLINIC_SERIAL_PUBLIC_KEY` env var, or the baked constant `_BAKED_SERIAL_PUBLIC_KEY` in `dental_clinic.py`). Generate the keypair once:
   ...
   **Activation flow (short serial, online):** vendor mints a serial in the GUI and **publishes it to the cloud** ("Upload minted serials to cloud" — `POST /api/license/admin/register-serial`, admin-token gated), which caches the signed token in the cloud registry. The owner is given only the short **Activation Code** — the ~22-char serial itself (e.g. `DENTAL-SMD-CLINI-00001`). On first run, after the staff sign-in, the app presents a premium, theme-aware activation card: the owner types the short serial and presses **Activate**. The app calls the cloud's `POST /api/license/claim`, which returns the cached signed token + atomically claims a device slot; the app then verifies that token locally (Ed25519) and **caches it on disk**, so every later launch works fully offline. **Internet is required only for that first activation.** An **"Activate offline (paste full code)"** fold remains for air-gapped installs — paste the full long token and it verifies locally with no network. The same card is reachable later from the **Renew** banner (grace period) or the view-only banner (inactive license). Signing is mandatory — there is **no** demo/default key. A self-contained, offline-verifiable code can't be short (an Ed25519 signature alone is ~86 chars), which is exactly why the short path goes through the cloud. See [`docs/SERIAL_GENERATOR_README.md`](docs/SERIAL_GENERATOR_README.md).
...
**1132 tests across 120 suites.** Covers the appointment API + flow, date utilities, the catalog migration, follow-up running balance, the per-patient combined payment history (follow-up sheet payments merged with billing records, ordering, totals, per-patient scoping, exclusion of zero-value and deleted entries), the unified patient balance (charged − paid summed across the follow-up sheet + billing, with credit = overpayment), expression preservation in money fields, appointment status updates, sync tombstones (delta export + deletion propagation), sync resilience (per-row error isolation, mobile-shaped payloads, billing `amount`), cloud-mode multi-tenant routing + tenant isolation + rate limit + Ed25519-signed serials, the local ⇄ cloud background sync round-trip, the per-tenant cloud backup loop (master + each `clinic_<id>.db`, per-label retention, isolation on per-tenant failure) plus the historic flat single-tenant layout, the Bluetooth-SPP fallback (4-byte length-prefixed frame codec, hello/bt_pair/sync_export/sync_import dispatcher reusing the HTTP helpers — including the zero-code first-time pair that issues a fresh device_token over the OS-bonded BT channel and rotates cleanly on re-pair, full session driver including malformed-frame handling, `/api/bt/status` + `/api/bt/configure` endpoints behind staff login, the `_BtSocketStream` adapter that lets the native RFCOMM listener reuse `_bt_serve_session` verbatim — `tests/test_bt_socket_stream.py`, and a daemon-thread worker that prefers the native AF_BTH path and falls back to the legacy pyserial COM-port path on `OSError` — `tests/test_bt_worker.py`), the BT diagnostics surfacing on `/api/bt/status` (paired_devices list, recent_attempts ring buffer bounded to maxlen=20, ok/unauthorized outcomes recorded by the dispatcher, server_listening flag round-trip — `tests/test_bt_diagnostics.py`), medical-image upload + byte download + sync reconciliation, the `/healthz` probe (200 with `status/mode/db_writable/uptime_seconds` on local, 503 when the DB is unreachable, open without a clinic token on the cloud node), the packaging/runtime plumbing (data-dir resolution for source vs. frozen/service builds, packaged-service-mode detection, the service health-check helper, and pywebview window-state persistence), and a property-fuzz suite that exercises every public endpoint (the odontogram routes included) with malformed JSON, wrong types, missing fields and oversized payloads — anything returning HTTP 5xx is a test failure. The odontogram backend contributes five suites: the editable tooth-condition catalog (CRUD, duplicate/blank rejection, soft-delete + tombstone), the tooth-chart **multi-condition replace-set** — multiple conditions per tooth, each with its own note, with legacy single-condition tolerance, dedupe, FDI validation and per-patient scoping, the read-time `has_plan`/`unpaid_balance` badge computation (with empty `conditions[]` on auto-adopted legacy `tooth_no` teeth), multi-tooth treatment plans via the `treatment_plan_teeth` link table (create / diff / cascade-delete, all tombstoned), and the export/import + tombstone round-trip for the three new `SYNC_TABLES` entries. Two cloud suites round it out: the one-tap pairing-QR endpoint (`tests/test_cloud_pairing_qr.py` — login gate, `400` when unpaired, `404` on the cloud node, and a well-formed SVG payload carrying the `{v,u,t}` token), and the tenant-DB backup sidecar (`tests/test_cloud_backup.py` — 14 cases over retention edge cases, the online-backup snapshot round-trip, and a single `run_once`). The cloud license authority adds two suites: `tests/test_serial_ed25519.py` (Ed25519 keypair + serial-token sign/verify, the `dental_clinic` verifier, `load_private_seed`, and the guarantee that there is no demo-key fallback) and `tests/test_license_authority.py` (`/api/license/validate` — signature verify, register-on-first-use, status/subscription/renewal gates including revoked-ignores-a-newer-token, the atomic device-slot cap under 8-thread concurrency, the `X-Admin-Token`-gated revoke/release endpoint, and that ProxyFix is wrapped only in cloud mode). Two premium-onboarding suites complete the backend coverage: `tests/test_onboarding_b.py` (baked cloud-URL fallback, `CLINIC_LICENSE_CLOUD_URL` env override, pair flow using the baked URL when omitted, onboarding-state endpoint, and the `_license_cloud_url` helper) and `tests/test_onboarding_ui_b.py` (cloud-link panel elements present in the HTML template + `node --check` syntax pass). A patient-create contract suite (`tests/test_patient_create_response.py`) pins that `POST /api/patients` echoes the created row (id + names, keeping `success: true` for the web portal) so the mobile app reconciles its local temp row to the server id instead of rebuilding a blank patient from a bare `{success: true}`. Three Data-Tools suites cover the database import/merge/replace surface: `tests/test_db_merge.py` (the additive cross-clinic merge engine — colliding-id re-numbering with current data preserved, FK + soft-link remaps for `expenses.reference_id`→follow-up and `credit.invoice_id`→billing, catalog dedupe by name, medical-image file copy + path rewrite, schema-drift tolerance, and a garbage-source safety check), `tests/test_db_import.py` (strict `SQLite format 3` magic validation + zip-slip-safe bundle build/extract), and `tests/test_data_tools_api.py` (the `/api/data/export-bundle · export-bundle-file · merge · replace · clear-catalogs` routes — login gate, cloud-node 404, non-SQLite/zip-slip rejection, `MAX_CONTENT_LENGTH` 413 cap, automatic safety backup, the additive merge through the HTTP layer, the live-swap replace + maintenance guard, the abort-if-backup-failed / no-concurrent-replace guards, clear-catalogs soft-deleting + tombstoning every procedure and condition while keeping patient data, the disk-export route writing a zip to `exports/`, the merge surfacing the real exception text and isolating a failing dependent table without aborting the merge, and a guard that the removed bulk clear-billing route is gone — 404). A `tests/test_seed_empty.py` suite pins that a fresh database ships with **empty** procedure and tooth-condition catalogs (no demo seed; the reserved `id=0` sentinel ships inactive). A `tests/test_license_claim.py` suite covers **short-serial online activation** end-to-end: the cloud `/api/license/claim` (cached-token lookup, `not_found`/`no_token`/revoked/expired gates, the atomic device-cap with idempotent re-claim), the admin-token-gated `/api/license/admin/register-serial` vendor pre-load (idempotent, garbage-rejecting, cloud-only), register-time token pre-loading, and the local server's `{serial_number}` activate path (delegates to the signed-token flow, 503-falls-back when the cloud is unreachable, passes business `reason` through, and leaves the air-gapped paste + LAN-attach routes unchanged). The vendor console adds upload coverage in `tests/test_serial_admin_d.py` (input validation, the per-serial result roll-up, that it POSTs each `serial_token` to `/api/license/admin/register-serial` with `X-Admin-Token`, and the loopback guard). The email-auth + per-dentist-accounts branch adds a further block of suites: OTP issue/verify/lockout and the admin recovery code (`tests/test_auth_codes.py`, `tests/test_recovery_code.py`), the bilingual email templates and the local↔cloud relay client (`tests/test_system_email_client.py`), login lockout + forgot/reset password + email set/verify + staff invites + admin security alerts end-to-end (`tests/test_login_email_lockout.py`, `tests/test_password_reset_flow.py`, `tests/test_email_verification.py`, `tests/test_invite_and_alerts.py`), and server-enforced per-dentist data scoping across appointments, billing/follow-ups, reports and the dashboard, plus the role-aware Manage Staff UI (`tests/test_role_scoping_appointments.py`, `tests/test_role_scoping_billing_followups.py`, `tests/test_role_scoping_reports.py`, `tests/test_role_ui.py`).

The Flutter app has its own analyzer-clean test suite (166 tests) under `clinic_mobile_app/test/` — currently `cloud_claim_test.dart` (`CloudSyncService.claim` posts the upper-cased serial + device fingerprint to `/api/license/claim`, returns null when the cloud is unreachable, and passes a `not_found` business failure straight through — backs the phone's short-serial activation, where `activateWithKey` falls back to a cloud claim when the entered code is a bare serial rather than a full signed token), `patient_name_test.dart` (pure `resolvePatientName` — synced appointment rows arrive carrying only `patient_id`, so the name is resolved from the local `patients` join instead of rendering as "Patient #N"; prefers a stored name, falls back to first+last, treats blank/`"null"` as missing — plus `patientInitials`, crash-safe avatar initials that fall back to `?` instead of indexing an empty string, since an empty name used to `RangeError` and white-screen the patients list), `patient_sync_test.dart` (`reconcileCreatedPatient` keeps the user-typed fields and only adopts a server-supplied id, so a thin `{success:true}` create response never blanks the just-added patient), `sync_banner_test.dart` (the top sync banner announces a *change* then auto-hides — `synced` after 3s, `offline`/`error` after 5s — and identical consecutive statuses are dropped so it stops re-flashing "Synced/Offline" on every routine cycle), `clinic_profile_test.dart` (`resolveDoctorName` — shows the Arabic name in Arabic, English otherwise, falling back across languages; backs the in-app editable doctor name that persists locally and syncs to `/api/clinic-settings`), `license_display_test.dart` (`maskSerial` shows only the last 4 of the serial; `licenseExpiryDate` pulls the `YYYY-MM-DD` from the expiry the activation key embeds — backs the Settings → License card's status / serial / "valid until"), `bluetooth_frame_codec_test.dart`, `bt_session_client_test.dart` (includes BT auto-pair handshake), `bluetooth_sync_service_test.dart` (includes auto-pair + self-heal on revoked token), `bt_error_message_test.dart` (`BtFailure` enum → bilingual EN/AR plain-language text via `btMessageFor`, plus `classifyBtError` mapping of `TimeoutException` / `'connect failed'` / `'read failed'` exception shapes), `followup_balance_test.dart`, `amount_expr_test.dart` (safe "20+20" money-expression evaluator), `patient_statement_totals_test.dart` (statement/invoice totals parity, to-pay/left clamped ≥0), `medical_image_reconcile_test.dart` (pull-side server↔local image reconciliation), `tooth_models_test.dart` (`ToothCondition` / `ToothChartEntry` JSON round-trip + `TreatmentPlan.teeth`), `tooth_chart_parse_test.dart` (pure `parseToothChart` splitting conditions + per-tooth badge entries), `odontogram_view_test.dart` (the arch renders 32 tooth cells + the condition legend), `pairing_payload_test.dart` (pure `parsePairingPayload` — decodes the desktop QR's `{v,u,t}`, rejects malformed/unsupported-version/blank payloads), `cloud_link_with_token_test.dart` (`linkWithToken` validates inputs and returns account info to persist, with **no** `/api/clinics/register` HTTP call), `license_gate_service_test.dart` (5 tests — LicenseGateService blocks navigation when unlicensed, passes when licensed, and honours the grace-period flag), and the default widget test. Run with `cd clinic_mobile_app && flutter test`.
