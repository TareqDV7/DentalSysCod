...
- The web portal (`/`), printable invoices (`/invoice/<id>`), the database backup download (`/api/backup`), and **every destructive Data Tools route** (`/api/data/export-bundle · merge · replace · clear-catalogs · clear-billing`) require a logged-in staff session — anonymous browsers are redirected to a sign-in page (API callers get `401`)
...
│   ├── test_data_tools_api.py      # /api/data/export-bundle · merge · replace · clear-catalogs · clear-billing (auth, cloud-disabled, size cap, safety backup, maintenance guard)
...
| POST | `/api/data/clear-billing` | Delete + tombstone every billing row (discard legacy/test billing after the unified-ledger move); follow-up sheets + patients untouched — login, local server only |
