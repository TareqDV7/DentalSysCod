...
- The web portal (`/`), printable invoices (`/invoice/<id>`), the database backup download (`/api/backup`), and **every destructive Data Tools route** (`/api/data/export-bundle · merge · replace · clear-catalogs`) require a logged-in staff session — anonymous browsers are redirected to a sign-in page (API callers get `401`)
